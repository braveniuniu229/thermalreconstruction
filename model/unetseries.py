...
class UNetGappyPOD(nn.Module):
    ...
    def forward(self, x):
        ...
        mask = self.mask(torch.cat([F.interpolate(dec2, enc1.size()[-2:], mode='bilinear',align_corners=True), enc1], 1))
        res = self.weight(torch.cat([F.interpolate(dec2, enc1.size()[-2:], mode='bilinear',align_corners=True), enc1], 1))
