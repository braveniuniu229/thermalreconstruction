...
import time
import os
from datasetvitincontext import dataset_train,dataset_test
from mlp import MLP
import csv
from incontextvit import IncontextViT
from torch.optim.lr_scheduler import LambdaLR
import wandb

old_checkpoint = torch.load('cnnvit/checkpoint_best.pth')
old_state_dict = old_checkpoint['model_state_dict']
new_state_dict = {}

for key in old_state_dict:
    new_key = key
    # 移除前缀 'model.'，因为新模型中不再使用它
...
model = IncontextViT(image_size=(64, 64), patch_size=(4, 4), num_classes=4096, dim=256, depth=7, heads=8, mlp_dim=4)
train_loader = DataLoader(dataset_train,batch_size=32,shuffle=True)
test_loader = DataLoader(dataset_test,batch_size=32,shuffle=False)
file = 'incontextvit'
"""这里每次都要修改成训练的model"""
checkpoint_dir = "incontextvit"   #这里修改成训练的断点
...
def train(epoch):
    ...
    for iteration, (combined_data,sample_label,final_prompt) in enumerate(train_loader):
        combined_data, sample_label, final_prompt = combined_data,sample_label,final_prompt
        combined_data,sample_label,final_prompt = combined_data.to(device).to(torch.float32),sample_label.to(device).to(torch.float32),final_prompt.to(device).to(torch.float32)

        optimizer.zero_grad()
        outputs = model(combined_data,final_prompt)
        labels = sample_label.view(outputs.size(0), -1)
...
def validate(epoch, best_loss):
    ...
    with torch.no_grad():
        ...
        for iteration, (combined_data, sample_label, final_prompt) in enumerate(test_loader):
            combined_data, sample_label, final_prompt = combined_data, sample_label, final_prompt
            combined_data, sample_label, final_prompt = combined_data.to(device).to(torch.float32), sample_label.to(device).to(torch.float32), final_prompt.to(device).to(torch.float32)
            outputs = model(combined_data,final_prompt)
            labels = sample_label.view(outputs.size(0), -1)
...
if __name__ == "__main__":
    best_loss = float('inf')
    num_epochs = 100 # 假设训练 30 个 epochs
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device).to(torch.float32)
    model.load_state_dict(new_state_dict,strict=False)
    print(f'model has load old pretrain model from{new_state_dict}')
